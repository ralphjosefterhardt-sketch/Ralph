...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function validateStep(step) {
    switch (step) {
      case 1:
        if (!state.selectedLanguage) { LF.Toast.warning('Bitte wähle eine Sprache aus.'); return false; }
        return true;
      case 2:
        if (!state.selectedLessonType) { LF.Toast.warning('Bitte wähle einen Kurstyp aus.'); return false; }
        return true;
      case 3:
  ...
  document.querySelectorAll('[data-step-prev]').forEach(btn => {
    btn.addEventListener('click', () => { state.currentStep--; updateStepUI(); });
  });
  ...
  if (bookBtn) {
    bookBtn.addEventListener('click', () => {
      ...
      setTimeout(() => {
        const booking = {
          language: state.selectedLanguage, type: state.selectedLessonType,
          date: state.selectedDate, time: state.selectedTime,
          price: calculateTotal(), coupon: state.couponCode,
          id: `BK${Date.now().toString(36).toUpperCase()}`, booked: new Date().toISOString()
        };
