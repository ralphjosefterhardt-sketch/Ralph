...
var ServiceReport = {
      ...
      <form id="serviceReportForm" class="report-form" novalidate>
        ...
        <fieldset class="form-section">
          ...
          <div class="checkbox-group">
            <label class="checkbox-label"><input type="checkbox" name="arbeiten" value="Inspektion / Wartung" /><span>Inspektion / Wartung</span></label>
            <label class="checkbox-label"><input type="checkbox" name="arbeiten" value="Reparatur" /><span>Reparatur</span></label>
            <label class="checkbox-label"><input type="checkbox" name="arbeiten" value="Ersatzteilwechsel" /><span>Ersatzteilwechsel</span></label>
            <label class="checkbox-label"><input type="checkbox" name="arbeiten" value="Softwareupdate" /><span>Softwareupdate</span></label>
            <label class="checkbox-label"><input type="checkbox" name="arbeiten" value="Einweisung" /><span>Einweisung</span></label>
            <label class="checkbox-label" id="sonstigesLabel"><input type="checkbox" name="arbeiten" value="Sonstiges" id="cbSonstiges" /><span>Sonstiges</span></label>
          </div>
        ...
        <fieldset class="form-section">
          ...
          <div class="form-group">
            ...
            <textarea id="beschreibung" name="beschreibung" rows="5" placeholder="Detaillierte Beschreibung..." required></textarea>
          </div>
        </fieldset>
        ...
        row.innerHTML = `
          ...
          <div class="ersatzteil-manual" style="display:none;margin-top:8px;">
            <div class="ersatzteil-fields">
              <div class="form-group"><label>Bezeichnung</label><input type="text" name="ersatzteil_bezeichnung[]" placeholder="Teilebezeichnung" /></div>
              <div class="form-group"><label>Artikelnummer</label><input type="text" name="ersatzteil_artikelnummer[]" placeholder="Art.-Nr." /></div>
            </div>
          </div>
        `;
        row.querySelector('.ersatzteil-artikel-select').addEventListener('change', function() {
          row.querySelector('.ersatzteil-manual').style.display = (this.value === '::sonstiges') ? 'block' : 'none';
        });
      } else {
        row.innerHTML = `
          <div class="ersatzteil-fields">
            <div class="form-group"><label>Bezeichnung</label><input type="text" name="ersatzteil_bezeichnung[]" placeholder="Teilebezeichnung" /></div>
            <div class="form-group form-group-narrow"><label>Menge</label><input type="number" name="ersatzteil_menge[]" min="1" value="1" placeholder="1" /></div>
            <div class="form-group"><label>Artikelnummer</label><input type="text" name="ersatzteil_artikelnummer[]" placeholder="Art.-Nr." /></div>
  ...
  _initCanvas: function(canvasId, stateKey, statusId, clearBtnId) {
    ...
    function onPointerUp(e) {
      ...
      if (statusEl) { statusEl.textContent = 'Unterschrift vorhanden ✓'; statusEl.className = 'sig-status sig-ok'; }
    }
    ...
    rows.forEach(function(row) {
      ...
      if (artikelSelect) {
        ...
        if (bezeichnung || artikelnummer) {
          ersatzteile.push({ bezeichnung: bezeichnung, menge: menge ? parseInt(menge.value, 10) || 1 : 1, artikelnummer: artikelnummer });
        }
      } else {
        var bez = row.querySelector('input[name="ersatzteil_bezeichnung[]"]');
        var artNr = row.querySelector('input[name="ersatzteil_artikelnummer[]"]');
        if (bez && bez.value.trim()) {
          ersatzteile.push({ bezeichnung: bez.value.trim(), menge: menge ? parseInt(menge.value, 10) || 1 : 1, artikelnummer: artNr ? artNr.value.trim() : '' });
        }
      }
    });
    ...
    overlay.innerHTML = `
      <div class="save-dialog">
        ...
        <div class="save-dialog-actions">
          ...
          <button class="btn btn-outline btn-full" id="dialogBackBtn">Zur&uuml;ck zur Maschine</button>
        </div>
      </div>
    `;
  ...
  showPrintView: function(report) {
    ...
    var arbeitenHtml = (report.arbeiten && report.arbeiten.length > 0)
      ? '<ul class="print-arbeiten-list">' + report.arbeiten.map(function(a) { return '<li class="print-arbeiten-tag">' + escapeHtml(a) + '</li>'; }).join('') + '</ul>'
    ...
    var fotosHtml = (report.fotos && report.fotos.length > 0)
      ? '<div class="print-photo-grid">' + report.fotos.map(function(f, i) { return '<img src="' + f + '" alt="Foto ' + (i + 1) + '">'; }).join('') + '</div>'
    ...
    var contentHtml = `
      ...
      <div class="print-report">
        ...
        <div class="print-section"><div class="print-section-title">Auftragsdaten</div><dl class="print-grid"><dt>Techniker</dt><dd>${escapeHtml(report.techniker)}</dd><dt>Datum</dt><dd>${formatDate(report.datum)}</dd>${report.auftragsnummer ? '<dt>Auftragsnummer</dt><dd>' + escapeHtml(report.auftragsnummer) + '</dd>' : ''}</dl></div>
        <div class="print-section"><div class="print-section-title">Maschinendaten</div><dl class="print-grid"><dt>Maschine</dt><dd>${escapeHtml(report.machineName || '')}</dd><dt>Seriennummer</dt><dd>${escapeHtml(report.serialNumber || '')}</dd><dt>Kunde</dt><dd>${escapeHtml(report.customerName || '')}</dd>${machine ? '<dt>Adresse</dt><dd>' + escapeHtml(machine.customer.address) + '</dd>' : ''}</dl></div>
        <div class="print-section"><div class="print-section-title">Durchgef&uuml;hrte Arbeiten</div>${arbeitenHtml}</div>
        <div class="print-section"><div class="print-section-title">Feststellungen &amp; Ma&szlig;nahmen</div><p style="font-size:0.88rem;color:#1f2937;line-height:1.6;white-space:pre-wrap;">${escapeHtml(report.beschreibung || '')}</p></div>
        <div class="print-section"><div class="print-section-title">Ersatzteile</div>${ersatzteileHtml}</div>
        ${(report.naechsterService || report.naechsterServiceBemerkungen) ? '<div class="print-section"><div class="print-section-title">N&auml;chster Service</div><dl class="print-grid">' + (report.naechsterService ? '<dt>Datum</dt><dd>' + formatDate(report.naechsterService) + '</dd>' : '') + (report.naechsterServiceBemerkungen ? '<dt>Bemerkungen</dt><dd>' + escapeHtml(report.naechsterServiceBemerkungen) + '</dd>' : '') + '</dl></div>' : ''}
        <div class="print-section"><div class="print-section-title">Fotodokumentation</div>${fotosHtml}</div>
        <div class="print-section"><div class="print-section-title">Unterschriften</div><div class="print-sig-row"><div class="print-sig-box"><div class="print-sig-label">Unterschrift Kunde</div><img src="${report.unterschriftKunde || ''}" alt="Unterschrift Kunde" /></div><div class="print-sig-box"><div class="print-sig-label">Unterschrift Techniker</div><img src="${report.unterschriftTechniker || ''}" alt="Unterschrift Techniker" /></div></div></div>
      </div>
    `;

    Machines._renderShell(contentHtml);
    document.getElementById('doPrintBtn').addEventListener('click', function() { window.print(); });
    document.getElementById('printBackBtn').addEventListener('click', function() { history.back(); });
  }
};
