...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function getFollowingLessonDate() {
    const d = new Date();
  ...
  const mockLessons = [
    {
      date: getNextLessonDate(),
      time: '17:00',
      type: 'Konversationsstunde',
      language: 'Englisch',
      trainer: 'Ralph Terhardt',
      id: 'ls001'
    },
    {
      date: getFollowingLessonDate(),
      time: '09:30',
      type: 'Grammatik-Vertiefung',
  ...
  const mockHomework = [
    {
      id: 'hw1',
      text: 'Present Perfect – Übungen Seite 45-47',
      subject: 'Englisch – Grammatik',
      due: 'Morgen',
      done: false
    },
    {
      id: 'hw2',
      text: '10 neue Vokabeln aus "Business English" lernen',
      subject: 'Englisch – Vokabular',
      due: 'In 3 Tagen',
      done: false
    },
    {
      id: 'hw3',
      text: 'Kurztext über deinen Alltag auf Französisch schreiben (150 Wörter)',
      subject: 'Französisch – Schreiben',
      due: 'In 4 Tagen',
      done: true
    },
    {
      id: 'hw4',
      text: 'Audio-Übung: Phonetik Kapitel 3 dreimal anhören',
      subject: 'Englisch – Aussprache',
      due: 'Diese Woche',
      done: false
    }
  ];
  ...
  if (storedUser) {
    ...
    if (storedUser.languages) {
      mockStudent.language = storedUser.languages.includes('french') ? 'Englisch & Französisch' : 'Englisch';
    }
  }
  ...
  userNameEls.forEach(el => { el.textContent = mockStudent.name; });
  ...
  function renderStats() {
    const statMap = {
      'stat-streak': `${mockStudent.streakDays} Tage`,
      'stat-hours': `${mockStudent.totalHours} Std.`,
      'stat-vocab': mockStudent.vocabularyLearned.toLocaleString('de'),
      'stat-next': ''
    };
  ...
  function renderHomework() {
    ...
    mockHomework.forEach(hw => {
      if (storedDone.includes(hw.id)) hw.done = true;
    });
    ...
    container.querySelectorAll('input[type=checkbox]').forEach(cb => {
      cb.addEventListener('change', () => {
        ...
        if (cb.checked) {
          ...
          const done = JSON.parse(localStorage.getItem('lf_homework_done') || '[]');
          const updated = done.filter(id => id !== hwId);
          localStorage.setItem('lf_homework_done', JSON.stringify(updated));
        }
      });
    });
  }
  ...
  // ============================================================
  // Live stat update (streak counter animation)
  // ============================================================
  ...
  function animateNumber(el, target, duration = 800) {
    ...
    const step = (timestamp) => {
      ...
      el.textContent = Math.round(start + (target - start) * progress).toLocaleString('de');
