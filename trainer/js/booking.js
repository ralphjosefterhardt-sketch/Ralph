...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Mock available days (days of month that have slots)
  ...
  // Mock booked slots (varies by day)
    ...
    switch (step) {
      case 1:
        if (!state.selectedLanguage) {
          LF.Toast.warning('Bitte wähle eine Sprache aus.');
          return false;
        }
        return true;
      case 2:
        if (!state.selectedLessonType) {
          LF.Toast.warning('Bitte wähle einen Kurstyp aus.');
          return false;
        }
        return true;
      case 3:
        if (!state.selectedDate) {
          LF.Toast.warning('Bitte wähle einen Termin aus.');
          return false;
        }
        if (!state.selectedTime) {
          LF.Toast.warning('Bitte wähle eine Uhrzeit aus.');
          return false;
        }
        return true;
      default:
  ...
  function updateStepUI() {
    ...
    document.querySelectorAll('.step').forEach((step, index) => {
      ...
      // Each .step is followed by a .step-line (except last), so index directly maps
  ...
  function renderCalendar(year, month) {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      if (isAvailable) {
        ...
        dayEl.addEventListener('click', () => selectDate(year, month, d, dayEl));
      }

      // Re-select if same date
      if (state.selectedDate) {
        const sd = state.selectedDate;
        if (sd.d === d && sd.m === month && sd.y === year) {
  ...
  function renderSummary() {
    ...
    const typeNames = {
      ...
      konversation: 'Konversationsstunde',
    ...
    const summaryFields = {
      ...
      'summary-date': state.selectedDate
        ? `${state.selectedDate.d}.${state.selectedDate.m + 1}.${state.selectedDate.y}`
        : '-',
  ...
  function calculateTotal() {
    return state.basePrice * (1 - state.discount / 100);
  }

  function getDiscountAmount() {
    return state.basePrice * (state.discount / 100);
  }
  ...
  if (couponBtn) {
    couponBtn.addEventListener('click', () => {
      ...
      if (!code) {
        LF.Toast.warning('Bitte gib einen Gutscheincode ein.');
        return;
      }

      if (couponCodes[code]) {
        state.discount = couponCodes[code];
        state.couponCode = code;
        LF.Toast.success(`Gutschein angewendet: ${state.discount}% Rabatt!`);
        renderSummary();
        couponBtn.textContent = 'Angewendet ✓';
