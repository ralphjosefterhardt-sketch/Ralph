...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (loginForm) {
    loginForm.addEventListener('submit', (e) => {
      ...
      if (!validatePassword(password.value)) {
        showFieldError(password, 'Das Passwort muss mindestens 8 Zeichen lang sein.');
        valid = false;
      }
      ...
      setTimeout(() => {
        // Mock: retrieve registered user or create default
        const registered = localStorage.getItem('lf_registered_user');
        let user;
        if (registered) {
          const reg = JSON.parse(registered);
          if (reg.email === email.value.trim()) {
            user = reg;
          }
        }

        if (!user) {
          // Default user for any login
          user = {
            name: 'Max Mustermann',
  ...
  if (registerForm) {
    registerForm.addEventListener('submit', (e) => {
      ...
      if (!name.value.trim() || name.value.trim().length < 2) {
        showFieldError(name, 'Bitte gib deinen vollständigen Namen ein.');
        valid = false;
      }
      ...
      if (password.value !== confirm.value) {
        showFieldError(confirm, 'Die Passwörter stimmen nicht überein.');
        valid = false;
      }

      // Get selected languages
      const selectedLangs = [];
      registerForm.querySelectorAll('.lang-option.selected').forEach(opt => {
        selectedLangs.push(opt.getAttribute('data-lang'));
      });

      if (selectedLangs.length === 0) {
        LF.Toast.warning('Bitte wähle mindestens eine Sprache aus.');
        valid = false;
      }
  ...
  document.querySelectorAll('.social-login-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      setTimeout(() => {
        const user = {
          name: provider === 'Google' ? 'Maria Schmidt' : 'Thomas Weber',
